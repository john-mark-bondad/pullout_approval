...
$columns = array(
  ...
  5=>'disapproved_date',  //pending_date, dispending_date, cancelled_date
  6=>'disapproved_by',   //pending_by, dispending_by, cancelled_by
  7=>'status', //status
  8=>''
...
$q1=$this->db->query("
  select x.pullout_id as ap_pullout_id, pullout_num, x.date_requested, x.saved_by as saved_by, x.disapproved_date as disapproved_date, x.disapproved_by as disapproved_by, x.description, x.status,
  ...
  and a.remark='1' and status='2'  $srch
...
$disapproved_date=$r[$x]['disapproved_date']; 
...
$q2=$this->db->query("
  ...
  ");
$r2=$q2->result_array();
$disapproved_by_name=(count($r2)==0)?"N/A":$r2[0]['lname']." ".$r2[0]['ename'].", ".$r2[0]['fname']." ".$r2[0]['mname'];
...
$data[$x] = array(
  ...
  <input type='hidden' id='tbl_pending_by$x' value='$disapproved_by_name' />

  <input type='hidden' id='tbl_requested_by$x' value='$save_by_name' />

  <input type='hidden' id='tbl_disapproved_date$x' value='$disapproved_date' />
  ...
  4=>"$saved_by_name",
  5=>"$disapproved_date",
  6=>"$disapproved_by_name", // 'saved_by' this is pending status
  7=> "$stat",
  8=>"<button class='btn' style='width:100%; background:rgba(0,0,0,0);' onclick='view_disapproved($x)'><span class='glyphicon glyphicon-search'></span></button>",
  9=>"<button class='btn btn-success approvebtn' onclick='pullout_approval_process_save($x);' disabled style='display:none;'>Approve</button>",
  10=>"<button class='btn btn-success approvebtn' onclick='pullout_disapproval_process_save($x);' disabled style='display:none;' >Dispprove</button>"
);
